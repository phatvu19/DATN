...
const DataIn1Year = () => {
    ...
    const options: any = {
        ...
        xaxis: {
            categories: fullYearData ? fullYearData.map(item => `Tháng ${item.name}`) : []
        },
    ...
    const onChange1 = async (dates: any, dateStrings: any) => {
        ...
        let combinedData = dateRange.map(date => {
            const found = response?.original?.daily_revenues?.find((d: any) => d.date == date);
            return {
                date: date,
    ...
    const menu = (
        <Menu >
            <Menu.Item key="1"><i className=" ri-calendar-2-line mr-2 font-normal" onClick={showModal} >Xem doanh thu theo tháng</i></Menu.Item>
            <Menu.Item key="2"><i className=" ri-calendar-2-line mr-4 font-normal" onClick={showModal1} >Xem doanh thu theo khoảng ngày</i></Menu.Item>
        </Menu>
    );
    return (
        <>
            <div className="">
                <div className="bg-white shadow rounded-lg pt-8 pb-10">
                    <span className='font-bold text-sm pl-10 flex'>Doanh Thu Trong 1 Năm
                    ...
                    <Modal width={'70%'} title="Doanh thu cả tháng" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
                        ...
                        <br className='mt-4' />
                        {!months ? <span className='font-bold text-xl'>Hãy chọn tháng.</span> : <span className=' text-xl'>Doanh thu của tháng {months} là: <span className='font-bold text-xl'>{formatNumber(totalm ? totalm : 0)}đ </span></span>}
                    ...
                    <Modal width={'70%'} title="Doanh thu theo khoảng ngày" open={isModalOpen1} onOk={handleOk1} onCancel={handleCancel1}>
                        ...
                        <br className='mt-4' />
                        {khoangday ? <LineChart width={1050} height={280} data={khoangday}
                            margin={{ right: 54, left: -20, top: 20 }}>
                            <CartesianGrid strokeDasharray="3 3" />
